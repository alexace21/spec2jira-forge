...
> **Model caveat (v5.4.0).** The retention and ZDR-eligibility statements in this DPA
> (Sections 7 and 13) describe the Managed pipeline as it actually runs in this release: the
> **Anthropic Claude Sonnet model via the asynchronous Message Batches API**. If the Managed
> model is ever changed to a "Covered Model" (which may carry a fixed 30-day, never-ZDR
> retention) or moved to the **synchronous Messages API** (which is ZDR-eligible), the
> retention claims in Sections 7.2–7.3 and 13 **must be re-verified** against Anthropic's
> then-current policy before they are relied upon. **[PARTNER: verify]** at each model change.
...
- **Honest current behaviour (v5.4.0 — breakdown-only release).** A breakdown that is
  **generated but never pushed persists in the Customer's own Atlassian instance** (Forge KVS)
  until one of the following occurs: (a) the Customer pushes the breakdown to Jira (which
  triggers the `purgeJob` purge described above), (b) the Customer regenerates the breakdown
  for the same page (the prior content is overwritten by the new generation), or (c) the App is
  uninstalled (Forge clears the App's stored data for that instance). **There is no automatic,
  time-boxed sweep and no KVS TTL in this release** — Spec2Tickets does **not** claim that an
  abandoned, never-pushed breakdown is removed after any fixed number of days. (A scheduled
  cleanup sweep is a future-release item and is **not** part of v5.4.0; do not assert it here.)
...
- **Spec2Tickets may proactively submit a deletion request for the batch immediately after
  retrieving the breakdown results, to minimise the residual window at Anthropic** below the
  ~29-day default where technically possible.
...
- For the Sub-processor: under the **Anthropic Commercial Terms (§B)**, Anthropic **does not
  ...
  data-sharing/model-improvement setting (e.g. feedback-based training) has been enabled on
  the account.
...
satisfy access/erasure requests directly. In the v5.4.0 (breakdown-only) release, the
controller-driven erasure levers for the copy Spec2Tickets holds in Forge are:
- **(a) Pushing the breakdown to Jira** — this purges the stored page content and breakdown
  via `purgeJob` (and deletes the push-session record), removing the App-held copy; and
- **(b) Uninstalling the App** — Forge clears the App's stored data for that instance; and
- **(c) Editing or removing the source Confluence page** — which the Customer controls
  directly within its own Atlassian instance.

> **Note for v5.4.0:** **regenerating** the breakdown does **not** immediately purge the prior
> stored copy — in this release a regenerate **orphans** the previous job (its content is
> overwritten only when the new generation writes over it / on push / on uninstall), so
> "delete or regenerate the breakdown" is **not** offered here as a self-service erasure lever.
> Use levers (a)–(c) above. (Any future automatic cleanup sweep is out of scope for v5.4.0.)
...
- Spec2Tickets will notify the Customer **without undue delay** after becoming aware of a
  Personal Data Breach affecting Customer Content processed under the Managed tier, providing
  the information reasonably available to enable the Customer to meet its own notification
  obligations. As a **Processor**, Spec2Tickets' duty under Art. 33(2) GDPR is to notify the
  Customer (the Controller) **"without undue delay"**; the fixed **72-hour** clock in Art. 33(1)
  is the **Controller's** duty to its supervisory authority, **not** the Processor's. **[PARTNER:
  legal review]** — decide whether to commit contractually to a fixed maximum window (e.g. "and
  in any event within 48–72 hours of becoming aware") to support the Customer's downstream
  72-hour duty, or to keep the GDPR-accurate "without undue delay" standard alone.
...
The Managed tier involves **two** processing legs; their transfer posture differs:

- **Leg 1 — Customer (Controller, EEA) → Spec2Tickets (Processor, EEA).** Where both the
  Customer and Spec2Tickets are established **inside the EEA**, this leg is **not** a restricted
  third-country transfer, so **no SCCs are required** for it — it is governed by the Art. 28
  GDPR processor terms in this DPA alone. **[PARTNER: legal review]** — confirm Spec2Tickets'
  place of establishment is in the EEA (see Section 1); if Spec2Tickets or the Customer is
  outside the EEA/UK, this leg may itself become a restricted transfer requiring the appropriate
  SCC module (Module 2, controller-to-processor) or the UK Addendum.
- **Leg 2 — Spec2Tickets (Processor, EEA) → Anthropic PBC (Sub-processor, United States).**
  This **is** a restricted/third-country transfer under the GDPR and UK GDPR (and potentially
  to other regions per Anthropic's sub-processor list). It is covered by the **EU Standard
  Contractual Clauses (2021/914), SCC Module 3 (processor-to-processor)** and, for UK data, the
  **UK International Data Transfer Addendum** — **incorporated into the Anthropic Commercial
  Terms / Anthropic DPA** between Spec2Tickets and Anthropic (no separate signature).
  **[PARTNER: legal review]** — confirm Module 3 is the correct module for this leg at
  submission, and identify the governing law and competent supervisory authority required by
  the SCCs.
...
- **Transfer impact / supplementary measures.** **[PARTNER: legal review]** — maintain a Transfer
  ...
  See the companion **`docs/compliance/TIA-managed-v5.4.0.md`** for the v5.4.0 TIA draft.
- **Atlassian Forge** stores data within the Customer's chosen Atlassian region; data residency
  ...
  leaves the Atlassian boundary is the **Anthropic** call described above (Leg 2).
...
### Document control
- **Status:** DRAFT — pending `[PARTNER: legal review]`. **v5.4.0 reconciliation** (breakdown-only release).
- **Scope:** Managed tier only. BYOK is unaffected and remains the privacy-maximising option.
- **Source of facts:** Anthropic Commercial Terms (§A.1 — "power your own product"; §B no-training-by-default;
  §D.4 bare-resale restriction only), Anthropic Privacy Center / trust.anthropic.com, Anthropic Message
  Batches API retention (non-ZDR, ≤~29 days, vendor may proactively delete the batch after retrieval; flagged
  content up to ~2 years), Anthropic DPA + EU SCCs auto-incorporated into the Commercial Terms (no separate
  signature; Spec2Tickets→Anthropic = SCC Module 3, processor-to-processor), and the App's actual Forge KVS
  purge behaviour for v5.4.0 (`purgeJob` + push-session deletion on push; no automatic sweep / TTL for
  never-pushed breakdowns in this release). **[PARTNER: verify]** all external figures against the providers'
  then-current published policies at the time of publication. **Model caveat (Section 6):** retention facts
  hold for the Sonnet-via-Batches pipeline; re-verify if the Managed model changes to a Covered Model or to
  the synchronous Messages API.
- **Last updated:** 2026-06-17 (v5.4.0 reconciliation; original 2026-06-03 draft unchanged).
