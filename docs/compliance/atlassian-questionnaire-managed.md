...
| 13 | **Authentication model for Atlassian access.** | **`asUser()`** — the app acts with the **signed-in user's own Atlassian permissions** for Confluence reads and Jira writes; no shared service account. Managed-tier generate/push is **gated behind an active license/trial** (unlicensed/Free = generate+review path per the hybrid design). | **Same security model.** Note for the reviewer: this preserves the **already-approved `asUser()` model**; Managed did **not** switch to `asApp()` (which would reopen the service-account concern). **[PARTNER: verify]** the license-gating is implemented as designed. |
...
| Stores data outside Atlassian? | **Yes** (Anthropic; Managed = under our account). See A.1. | **Changed** (BYOK leaned on "under the customer's key"). |
...
3. **Retention is disclosed honestly.** We use the **Anthropic Batches API**, which is **not
   ZDR-eligible** (≤~29-day retention). We **deliberately do not claim zero retention.** A
...
- [x] **Backstop for abandoned/never-pushed breakdowns — DONE (Task #13, 2026-06-14):** a daily
      Forge scheduled sweep (`sweepHandler`) removes a never-pushed breakdown 7 days after its last
      access (access-renewed via `jobmeta.lastAccessedAt`; NOT a creation-anchored TTL). Disclosed in
      §7.1 + the site privacy/dpa/subprocessors. ([PARTNER: verify] post-push `purgeJob` on the Managed path.)
- [ ] **[PARTNER: verify]** Managed generate/push is gated behind an active license/trial;
      `asUser()` retained (no `asApp()`).
- [ ] **[PARTNER: legal review]** SCC module(s): Spec2Tickets→Anthropic (processor-to-processor)
      **and** customer↔Spec2Tickets (controller-to-processor); TIA on file.
...
- [ ] **[PARTNER: verify]** the **~29-day** retention and **~2-year** flagged-content figures
      against Anthropic's then-current published policy at submission time.
