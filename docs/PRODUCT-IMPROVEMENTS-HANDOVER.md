...
**Pricing (DECIDED 2026-06-02; prices finalized at resubmit).** **Managed Pro = €9.90/seat/mo** with
a **fair-use cap of 10 breakdowns per USER per month (metered per-user via `usage:YYYY-MM:u:<accountId>`,
NOT pooled — the License object exposes no runtime seat count)** — vs **BYOK Pro €4.90/seat unlimited**.
(≤10 floors: €99 Managed / €49 BYOK.) Grounding: Sonnet 4.6 batched ≈ €0.10/breakdown typical, €0.25
large (32K out). **Atlassian fee: Forge = 0% up to $1M lifetime (we keep 100%), then ~16-17%.**
Worst-case capped cost ≈ 10 × €0.25 = €2.50/user → net margin **~75% now / ~58% post-$1M**; ~85-90%
expected. **Cap=10 is loss-proof even in the all-max-64K pathological case** (10×€0.46 = €4.60 < €9.90,
positive in both phases). Chose 10 (not 15/20) to future-proof: lowering a cap later is
customer-hostile — start conservative, raise if needed. Cap reuses the `src/usage.js` counter
(per-user `MANAGED_USER_CAP`, default 10). Heavy users route to BYOK.
