...
1. **🔑 FUND + SET `MANAGED_ANTHROPIC_KEY` on production** (`forge variables set --environment production MANAGED_ANTHROPIC_KEY <key> --encrypt`) + `MANAGED_TRIAL_CREDIT_USD 5` + **`MANAGED_HARD_CEILING_USD 10`** (gate note: set the ceiling EXPLICITLY so a grant typo can't scale the absolute backstop). **This is #1** — the $5 trial runs on OUR key; unset → trial users hit `managed_unavailable` on generate (graceful, but no free trial). Prod was recommended UNSET until now.
