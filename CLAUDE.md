...
**Plan: ship a fresh, unambiguous version (6.6.0) and cite its exact build number to Nadiia**, rather than resubmitting into the same ambiguity. Branch `chore/version-bump-6.6.0-editions-unblock` (off `main`, 1 clean commit `86c9d7c`): package.json + `src/diagnostics.js` `DIAG_APP_VERSION` bumped 6.5.0→6.6.0, lockstep-verified (`npm run check` 11/11 + version-drift-guard green). **No dependency/code changes** — see the correction below for why.
...
**NEXT (partner-executed — production deploy is the partner's action, not Claude's):** push `chore/version-bump-6.6.0-editions-unblock` → PR → merge to `main` (branch protection now real — the PR gate matters) → trigger "Deploy to production" (`workflow_dispatch`, `version: 6.6.0`, `scope_ack: "no scope change"`) → confirm the new build number + date in the Versions tab → reply to Nadiia citing that exact build + manifest excerpt (draft prepared in-session) + politely note the docs' own 1-2-business-day SLA vs. the 2+ week wait.
