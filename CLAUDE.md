...
> ⚠ Branch `feature/UI-UX-improvements` — **DEV-deployed + partner live-accepted; NOT yet merged/prod.** One commit this session: **`264ea99`** (partner pushes + merges). Durable: [[deep-audit-vs-per-change-gate]] (vindicated AGAIN — a HIGH bug the per-change gate + offline reasoning missed) · [[moodboard-design-system]]. New source of truth = `static/hello-world/src/lib/pageOutline.js` (the client-side extraction) + `PreflightCard` in `App.js`.
...
- **NOT run yet: a correctness army on the new pill-rollup logic** (child counts / per-section TODO attribution / minLevel spans) — offered post-visual-signoff before any commit-to-prod. Consider running it next session.
