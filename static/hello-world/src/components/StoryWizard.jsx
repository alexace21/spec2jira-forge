...
// ── CoverageBadge — green=100% / orange=partial / grey=no-ACs (unchanged from the old card) ──────────────
function CoverageBadge({ coverage }) {
  ...
  if (no_acs) {
    return (
      <span className="text-[11px] px-1.5 py-0.5 rounded font-medium" style={{ background: "var(--s2j-bg-section)", border: "1px solid var(--s2j-border)", color: "var(--s2j-text-muted)" }} title="No acceptance criteria — all cases are inferred">no ACs</span>
    );
  }
  const pct = typeof coverage_pct === "number" ? coverage_pct : null;
  const isComplete = pct === 100 && staleCount === 0;
  const fg = isComplete ? "var(--s2j-green-dark)" : pct !== null ? "var(--s2j-orange)" : "var(--s2j-text-muted)";
  const bg = isComplete ? "var(--s2j-green-bg)" : pct !== null ? "var(--s2j-orange-bg)" : "var(--s2j-bg-section)";
  const border = isComplete ? "var(--s2j-green-border)" : pct !== null ? "var(--s2j-orange-border)" : "var(--s2j-border)";
  const label = pct !== null ? `${covered_acs}/${total_acs} ACs` : "–/– ACs";
  const title = pct !== null ? `${covered_acs} of ${total_acs} acceptance criteria covered${staleCount ? ` · ${staleCount} stale reference${staleCount > 1 ? "s" : ""}` : ""}` : "Coverage unknown";
  return (
    <span className="text-[11px] px-1.5 py-0.5 rounded font-medium inline-flex items-center gap-1" style={{ background: bg, border: `1px solid ${border}`, color: fg }} title={title}>
      {label}
      {staleCount > 0 && <span title={`${staleCount} AC reference(s) no longer match the current story`}><SignalIcon kind="warning" size={14} /></span>}
    </span>
  );
}
...
function StoryChips({ entry, isStale, isRemoved, isDirty, cases, distribution }) {
  ...
  return (
    <>
      {isRemoved && !hasError ? (
        <span className="text-[11px] px-1.5 py-0.5 rounded font-medium" style={{ background: "var(--s2j-red-bg)", border: "1px solid var(--s2j-red-border)", color: "var(--s2j-red)" }} title="This story was removed from the breakdown in the editor — its cases are orphaned and won't embed on push. Re-add the story; it cannot be regenerated.">
          <IconBan size={14} /> Removed from breakdown
        </span>
      ) : isStale && !hasError ? (
        <span className="text-[11px] px-1.5 py-0.5 rounded font-medium" style={{ background: "var(--s2j-orange-bg)", border: "1px solid var(--s2j-orange-border)", color: "var(--s2j-orange)" }} title="This story's acceptance criteria changed since these test cases were generated — Regenerate to refresh">
          <SignalIcon kind="warning" size={14} /> ACs changed
        </span>
      ) : null}
      {isDirty && (
        <span className="text-[11px] px-1.5 py-0.5 rounded font-medium" style={{ background: "var(--s2j-blue-bg)", border: "1px solid var(--s2j-blue-border)", color: "var(--s2j-blue)" }} title="This story has unsaved edits — Save to update coverage, export and push">● edited</span>
      )}
      {hasError && (
        <span className="text-[11px] px-1.5 py-0.5 rounded font-medium" style={{ background: "var(--s2j-red-bg)", border: "1px solid var(--s2j-red-border)", color: "var(--s2j-red)" }}>
          <SignalIcon kind="error" size={14} /> Failed
        </span>
      )}
      {!hasError && coverage && <CoverageBadge coverage={coverage} />}
      {!hasError && entry?.truncated === true && (
        <span className="text-[11px] px-1.5 py-0.5 rounded font-medium" style={{ background: "var(--s2j-orange-bg)", border: "1px solid var(--s2j-orange-border)", color: "var(--s2j-orange)" }} title="Generation hit the output limit for this story — complete cases were recovered, but the last ones may be missing. Regenerate to be sure.">
          <SignalIcon kind="warning" size={14} /> may be truncated
        </span>
      )}
      {!hasError && cases && cases.length > 0 && distribution && (
        <span className="text-[11px] px-1.5 py-0.5 rounded" style={{ background: "var(--s2j-bg-section)", border: "1px solid var(--s2j-border)", color: "var(--s2j-text-muted)" }} title="Case count by type">
          {distribution}
        </span>
      )}
    </>
...
// ════════════════════════════════════════════════════════════════════════════════════════════════════
// StoryRow — a compact overview row (replaces the <details> accordion). One vertical scan triages N stories;
// "Open →" drills into the per-story wizard. Per-row Regenerate + Copy so the BA never drills in to retry.
// ════════════════════════════════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div
      ...
      style={{
        ...
        border: "1px solid rgba(125,160,202,0.30)",
        borderLeft: `4px solid ${accent}`,
        borderRadius: 12,
        boxShadow: "0 4px 16px rgba(5,38,89,0.05)",
        padding: "10px 14px",
      }}
    >
...
// wiring is the SAME as the old card; only the shell changed.
// ════════════════════════════════════════════════════════════════════════════════════════════════════
export default function StoryWizard({
  ...
  if (hasError) {
    return (
      <div style={wrapStyle}>
        <button type="button" onClick={onBackToList} className="text-xs flex items-center gap-1.5" style={{ color: "var(--s2j-blue)", background: "none", border: "none", cursor: "pointer", padding: "4px 8px", marginLeft: -8, marginBottom: 8 }}>← All stories</button>
        <h3 style={{ fontSize: 18, fontWeight: 700, color: MOOD.navy, margin: "0 0 12px" }}>{storyName}</h3>
        <div className="rounded p-3 text-xs" style={{ background: "var(--s2j-red-bg)", border: "1px solid var(--s2j-red-border)", color: "var(--s2j-red)" }}>
          Generation failed — Regenerate to retry this story.
        </div>
  ...
  return (
    <div style={wrapStyle}>
      {/* header */}
      <button type="button" onClick={onBackToList} className="text-xs flex items-center gap-1.5" style={{ color: "var(--s2j-blue)", background: "none", border: "none", cursor: "pointer", padding: "4px 8px", marginLeft: -8, marginBottom: 6 }} title="Back to all stories">← All stories</button>
      ...
      {readOnly && (
        <div className="rounded-lg p-2.5 mb-3 text-xs" style={{ background: "var(--s2j-blue-bg)", border: "1px solid var(--s2j-blue-border)", color: "var(--s2j-text)" }}>
          <strong>Viewing your existing test cases.</strong> Editing and regenerating are an Advanced-edition feature — your saved cases stay viewable and exportable.
        </div>
      )}
      ...
      {result?.no_acs && (
        <p className="text-xs mb-3 italic" style={{ color: "var(--s2j-text-muted)" }}>This story had no acceptance criteria — every test case is inferred. Review before relying.</p>
      )}
      {isPolling && (
        <p className="text-xs mb-3 italic" style={{ color: "var(--s2j-text-muted)" }}><IconRefresh size={14} /> Regenerating this story — editing is paused until the new cases arrive…</p>
      )}
      {movedNote && (
      ...
      {phase === 4 && (
        <div>
          ...
          <div className="mb-3 text-[13px]" style={{ color: "var(--s2j-text-muted)" }}>
            <span style={{ fontWeight: 600, color: "var(--s2j-text)" }}>Test cases by type:</span>{" "}
            {TYPE_PHASES.map((t, idx) => {
              const cnt = byType(t).length;
              return (
                <span key={t} style={{ color: cnt === 0 ? "var(--s2j-orange)" : "var(--s2j-text-muted)" }}>
                  {idx > 0 ? " · " : ""}{TYPE_LABEL[t]} {cnt}{cnt === 0 ? " ⚠" : ""}
                </span>
              );
            })}
          </div>
          ...
          {!isDirty && saveState === "saved" && (
            <div className="mb-3 text-xs" style={{ color: "var(--s2j-green-dark)" }}><IconCheck size={14} /> Saved — coverage, export and push now use your edits.</div>
          )}
          ...
          {coverage && (uncovered.length > 0 || staleRefs.length > 0) ? (
            <div className="flex flex-col gap-2">
              ...
              {uncovered.length > 0 && (
                <div className="rounded-lg px-3 py-2.5 text-xs" style={{ background: "var(--s2j-orange-bg)", border: "1px solid var(--s2j-orange-border)" }}>
                  <p className="font-semibold mb-1 text-[13px]" style={{ color: "var(--s2j-text)" }}>Acceptance criteria without a test case ({uncovered.length})</p>
              ...
              {staleRefs.length > 0 && (
                <div className="rounded-lg px-3 py-2.5 text-xs" style={{ background: "var(--s2j-orange-bg)", border: "1px solid var(--s2j-orange-border)" }}>
                  <p className="font-semibold mb-1 text-[13px]" style={{ color: "var(--s2j-text)" }}>References that no longer match a current AC ({staleRefs.length}) — an AC may have been edited since generation</p>
                  <ul className="ml-3 list-disc text-[13px]" style={{ color: "var(--s2j-text-light)" }}>{staleRefs.map((ref, i) => <li key={i}>{ref}</li>)}</ul>
                </div>
              )}
            </div>
          ) : !result?.no_acs ? (
            <div className="rounded-lg px-3 py-2.5 text-[13px] flex items-center gap-2" style={{ background: "var(--s2j-green-bg)", border: "1px solid var(--s2j-green-border)", color: "var(--s2j-green-dark)" }}>
              <IconCheck size={15} /> Every acceptance criterion is covered{isDirty ? " (as of your last save — Save to recompute)" : ""}.
            </div>
          ) : null}
        </div>
      )}
