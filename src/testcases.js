...
// ════════════════════════════════════════════════════════════════
// renderTestCasesAdf — COMPACT SUMMARY ADF for Story description embed (P4 audit).
//
// Partner chose a compact summary (NOT the full verbose per-case rendering) so
// the JIRA Story description stays readable. Full cases + Gherkin/CSV export live
// on the dedicated P5 Test Cases screen.
//
// Returns Array<ADF content node> to spread into the Story description:
//   heading  h3 "Test Cases"
//   para 1   {N} test case{s} [· {covered}/{total} ACs covered | · no ACs (inferred)]
//   para 2   type breakdown: "{x} happy-path · {y} edge · {z} negative" [· {k} flagged]
//   para 3   em pointer to the Test Cases screen for the full export
//
// Safe node types ONLY (gotcha #11): heading/paragraph/text + strong/em.
// NO bulletList, NO codeBlock, NO panel — a rejected node fails EVERY Story create.
// Returns [] when result.test_cases is empty/absent → a no-op spread.
// Pure JSON builder, zero Node APIs — runs in the Forge backend.
// ════════════════════════════════════════════════════════════════

/**
 * Build a single ADF `paragraph` node containing one `strong` label followed by
 * a plain text value. Returns null when value is empty (caller omits it).
 * Helper retained for callers that may use it; not used internally by renderTestCasesAdf.
 */
export function adfLabelParagraph(label, value) {
...
/**
 * Render one Story's parsed test-case result as compact summary ADF nodes to
 * embed in the JIRA Story description. Safe node types only (gotcha #11).
 * Full cases + export live on the P5 Test Cases screen.
 *
 * @param {object} result   parsed TEST_CASE result ({ test_cases[], no_acs? })
 * @param {object} coverage computeCoverage() return ({ coverage_pct, covered_acs, total_acs, no_acs })
 * @returns {Array}         ADF content nodes — spread into `richADF`'s content array
 */
export function renderTestCasesAdf(result, coverage) {
  const r = result || {};
  const cases = Array.isArray(r.test_cases) ? r.test_cases.filter((c) => c && typeof c === 'object') : [];
  ...
  // ── Para 1: count + coverage ─────────────────────────────────
  // "{N} test case{s}" bold + coverage phrase plain.
  const countText = `${n} test case${n === 1 ? '' : 's'}`;
  let coverageText = '';
  if (coverage && typeof coverage.coverage_pct === 'number') {
    // coverage_pct is non-null ⇒ story has authored ACs
    coverageText = ` · ${coverage.covered_acs}/${coverage.total_acs} acceptance criteria covered`;
  } else if ((coverage && coverage.no_acs) || r.no_acs) {
    coverageText = ' · no acceptance criteria (inferred from the user story)';
  }

  const para1Content = [
    { type: 'text', text: countText, marks: [{ type: 'strong' }] },
  ];
  if (coverageText) {
    para1Content.push({ type: 'text', text: coverageText });
  }
  nodes.push({ type: 'paragraph', content: para1Content });

  // ── Para 2: type breakdown + flagged count ───────────────────
  const typeCounts = { 'happy-path': 0, edge: 0, negative: 0 };
  let flaggedCount = 0;
  for (const c of cases) {
    const t = String(c.type || 'happy-path');
    if (t in typeCounts) typeCounts[t]++;
    if (c.concern) flaggedCount++;
  }
  const typeParts = [];
  if (typeCounts['happy-path'] > 0) typeParts.push(`${typeCounts['happy-path']} happy-path`);
  if (typeCounts.edge > 0) typeParts.push(`${typeCounts.edge} edge`);
  if (typeCounts.negative > 0) typeParts.push(`${typeCounts.negative} negative`);
  let para2Text = typeParts.join(' · ');
  if (flaggedCount > 0) para2Text += ` · ${flaggedCount} flagged`;
  // Guard blank (all cases have unrecognised types) — omit the paragraph entirely.
  if (para2Text) {
    nodes.push({ type: 'paragraph', content: [{ type: 'text', text: para2Text }] });
  }
  ...
  nodes.push({
    ...
    content: [
      {
        type: 'text',
        text: 'Generated by Spec2Tickets — the full cases and Gherkin / CSV export are available in Spec2Tickets during review and right after push.',
        marks: [{ type: 'em' }],
      },
    ],
  });
